...
const cidades = document.getElementById('form-cidade');
const cidadesSelected = document.getElementById('DataListCidade');
const cidadeObj = document.getElementById('Cidade');
...
function ChangeCidades(id) {
    $.ajax({
        url: '/Landingpage/AtualizaCidades',
        type: 'POST',
        data: { estadoId: id },
        dataType: 'json'
    }).done((response) => {
        if (response.sucesso) {
            response.cidades.forEach(c => {
                var option = document.createElement('option');
                option.text = c.cidadeNome;
                option.value = c.cidadeNome;
                option.setAttribute("data-index", c.id );
...
function AlteraCidades() {
    estados.addEventListener('change', (e) => {
        var estadoId = estados.value;
        if (cidades.childElementCount > 1) {
            $("#form-cidade").empty();
            cidadesSelected.value = "";
        }
...
function CidadeSelected() {
    cidadesSelected.addEventListener('change', () => {
        var valor = $('#form-cidade [value="' + cidadesSelected.value + '"]').attr('data-index')
        if (valor == null) {
            cidadesSelected.style.borderColor = "rgba(0, 0, 0, 0.5)";
            cidadesSelected.value = "";
            cidadeObj.value = null;
        }
        else {
            cidadesSelected.style.borderColor = "limegreen";
            cidadeObj.value = valor;
        }
    });
}
